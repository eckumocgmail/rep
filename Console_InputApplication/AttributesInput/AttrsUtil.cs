...
public partial class AttrsUtil: Utils
{
    ...
    public static Dictionary<string, Dictionary<string, string>> GetNavigationProperties(Type type) {
        ...
        GetNavigation(type).Select(s => s.Name).ToList().ForEach(key=> {
            result[key] = AttrsUtil.ForProperty(type, key);
        });
        return result;
    }
    ...
    {
        var navs = AttrsUtil.GetNavigation(ReflectionService.TypeForName(instance));
    ...
    public static bool IsManyToManyRelation(Type type, string propertyName) {
        return AttrsUtil.ForProperty(type, propertyName).ContainsKey(nameof(ManyToMany));
    }
...
*/
    ...
    {
        return AttrsUtil.ForProperty(type, bindingGroup)[nameof(ManyToMany)];
    }

    public static string GetControlType(Dictionary<string, string> attributes)
    {
        return attributes.Keys.FirstOrDefault(key => key.ToType().IsExtends(typeof(ControlAttribute)));
    }
}
