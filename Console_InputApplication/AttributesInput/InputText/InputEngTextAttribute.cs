...
public class InputEngTextAttribute : BaseInputAttribute, MyValidation
{
    ...
    {
        string alf = "_,.!?-;:()'\"() " + "qwertyuiopasdfghjklzxcvbnm" + " " + "qwertyuiopasdfghjklzxcvbnm".ToUpper();
        string text = value.ToString();
        if (text.SplitWords().Any(word => word.IsEng() == false && word.IsPuntuationsOrSplitters() == false))
            return false;
        return true;
    }
