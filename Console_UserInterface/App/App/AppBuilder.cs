...
public class AppBuilder
{
    ...
    public Dictionary<string, string> CreateNavMenu<T>(Dictionary<string, string> mapping = null)
    {
        ...
        var types = typeof(T).Assembly.GetTypes().Where(t => t.Namespace == snamespace && t.GetTypeAttributes().ContainsKey("RouteAttribute"));
    ...
    {
        Dictionary<string, string> result = new();
        var types = typeof( Console_UserInterface.Program).Assembly.GetTypes().Where(t => t.Namespace == snamespace && t.GetTypeAttributes().ContainsKey("RouteAttribute"));
        result = new Dictionary<string, string>(types.Select(type =>
        {
            var route = type.GetAttributes()["RouteAttribute"];
            if (mapping is not null)
