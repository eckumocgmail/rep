...
namespace pickpoint_delivery_service
{
    public class DeliveryDbContextInitiallizer : IdeliveryDbContextInitiallizer
    {
        ...
        public IDictionary<string, int> Init(DeliveryDbContext context, string contentPath)
        {
            ...
            this.Info("Проверка набора ProductCatalogs");
            result["ProductCatalogs"] = InitProductCatalogs(context, contentPath);
            this.Info("Проверка набора Products");
            result["Products"] = InitProducts(context, contentPath);
            this.Info("Проверка набора Holders");
            result["Holders"] = InitHolders(context);
            this.Info("Проверка набора Transport");
            result["Transport"] = InitTransport(context);
            this.Info("Проверка набора ProductsInStock");
            result["ProductsInStock"] = InitProductsInStock(context, 10);
            this.Info("Проверка набора Customers");
        ...
        private int InitProductCatalogs(DeliveryDbContext context, string contentPath)
        {
            int result = 0;
            if (context.ProductCatalogs.Count() == 0)
            {
                context.ProductCatalogs.ToList().ForEach(p => context.ProductCatalogs.Remove(p));
                context.SaveChanges();
                var root = context.ProductCatalogs.FirstOrDefault(p => p.ParentId == null);
                if(root is null)
                {
                     
                    context.ProductCatalogs.Add(root = new()
                    {
                        ProductCatalogName = "Товары",
                        ParentId = null
                    });
        ...
        private int InitProductsInStock(DeliveryDbContext context, int productsCount)
        {
            int result = 0;
            if(context.ProductsInStock.Count() == 0)
            {

                var random = new Random();
                foreach (var holder in context.Holders.ToList()) 
                {
                    for (int i = 0; i< productsCount; i++)
                    {
                        int index = random.Next(0, context.Products.Count()-1);
                        ...
                        {
                            context.HolderStorages.Add(new Blazor_UserInterface.AppUnits.DeliveryModel.HolderStorage()
                            {
                                HolderId = holder.Id,
                                ProductId = product.Id,
                                StorageCode = $"{context.HolderStorages.Where(hs => hs.HolderId == holder.Id).Count()}",
                                ProductCode = $"{DateTime.Now.ToString("d")}-{holder.Id}-{product.Id}-{j}"
                            });
                            result += context.SaveChanges();
                        }
        ...
        public int InitProducts(DeliveryDbContext context, string contentPath= @"D:\System-Config\MyExpirience\Console_BlazorAPp\Console_UserInterface\Resources\")
        {
            if (context.Products.Count() > 0)
                return 0;
            string ResourceDir = contentPath;
            ...
            PostSparePartsOfferResponse offer = new PostSparePartsOfferResponse()
            {
                Offer_lines =Newtonsoft.Json.JsonConvert.DeserializeObject<List<PostSparePartsOfferResponseItem>>(@" [
                    {
                      ""product_id"": ""0-0-5314403-0"",
                      ""price"": 200,
                      ""quantity"": 3
                    },
                    {
                      ""product_id"": ""0-0-1496638-0"",
                      ""price"": 210,
                      ""quantity"": 20
                    } 
            ...
            if (offer is not null && offer.Offer_lines is not null)
            foreach(var product in offer.Offer_lines)//new List<PostSparePartsOfferResponseItem>() { offer.Offer_lines.ToList()[0] , offer.Offer_lines.ToList()[1] })
            {
                    
                this.Info(product.Product_id);
                var item = spare.PartAsync(product.Product_id).Result.First(p => p.Product_id == product.Product_id);
                var random = new Random();
                var target = new Product()
                {
                    NumberOfSales = 0,
                    IsNew = (random.NextInt64(1, 2) == 2? true: false),                         
                    ProductRate = random.NextInt64(1,10),
